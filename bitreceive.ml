...
type config_t = {
  rpcip: string; rpcport: int; rpcuser: string; rpcpassword: string; nconfirm: int; state_file: string;
...
let mutex () =
  let mutex_file = "/tmp/bitreceive.lock" in
...
let scan_and_notify config state =
  ...
  let (min_safe_block_info, has_work, txs) = List.fold_left ~init:(None,false,[]) ~f:(fun acc tx -> 
      ...
      match (List.Assoc.find tx "category") with
        Some (`String "receive") -> 
        begin match (List.Assoc.find tx "address", List.Assoc.find tx "amount", List.Assoc.find tx "blockhash", List.Assoc.find tx "confirmations") with
            (Some (`String address), Some (`Float amount), Some (`String blockhash), Some (`Int confirmations)) -> 
            let tx_acc = `Assoc [
            ...
            let min_safe_block_info =
              if confirmations >= config.nconfirm then
                begin match min_safe_block_info with
                    None -> Some (confirmations, blockhash)
                  | Some (min_safe_block_nconfirm,min_safe_block) -> 
  ...
  Out_channel.write_all config.output_pipe ~data:json_string; 
